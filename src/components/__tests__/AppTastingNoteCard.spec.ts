...
import AppTastingNoteCard from '@/components/AppTastingNoteCard.vue';
import { Editable, TastingNote } from '@/models';

describe('Tasting Note Card', () => {
  let testNote: Editable<TastingNote>;
  const vuetify = createVuetify({ components, directives });
  const mountComponent = (note: Editable<TastingNote>) =>
    mount(AppTastingNoteCard, { global: { plugins: [vuetify] }, props: { note } });
